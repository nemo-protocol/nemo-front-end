...
>(coinConfig?: CoinConfig, debug: T = false as T) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { pyPosition, created } = await initPyPosition({tx, coinConfig, pyPositions})
      ...
      const returnValue = result?.results?.[index]?.returnValues?.[0]?.[0]
      ...
      const tradeFeeReturnValue = result?.results?.[index]?.returnValues?.[1]?.[0]

      if (!tradeFeeReturnValue) {
        const message = "Failed to get add liquidity data"
        debugInfo.rawResult.error = message
        throw new ContractError(message, debugInfo)
      }
      ...
      const tradeFeeAmount = bcs.U128.parse(
        new Uint8Array(tradeFeeReturnValue),
      )
