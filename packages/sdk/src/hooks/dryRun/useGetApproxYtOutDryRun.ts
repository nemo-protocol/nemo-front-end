...
export default function useGetApproxYtOutDryRun<T extends boolean = false>(
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const approxYtOut = bcs.U64.parse(new Uint8Array(returnValue))

      const returnValue1 =
        result?.results?.[result.results.length - 1]?.returnValues?.[1]?.[0]
      if (!returnValue1) {
        const message = "Failed to get net sy tokenization amount"
        debugInfo.rawResult.error = message
        throw new ContractError(message, debugInfo)
      }

      const netSyTokenization = bcs.U64.parse(new Uint8Array(returnValue1))
