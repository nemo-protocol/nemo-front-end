        ...
        return {
            ...
            yt: list.filter((item) => {
                return (
                    ...
                    (!NO_SUPPORT_YT_COINS.includes(item?.coinType) ||
        ...
        if (address && marketStates && pyPositionsMap && filteredLists) {
            ...
            filteredLists.pt.forEach((item) => {
                _balance = _balance.add(
                    new Decimal(pyPositionsMap?.[item?.id]?.ptBalance || 0).mul(
                        item.ptPrice
                    ) || 0
                )
            })
            filteredLists.yt.forEach((item) => {
                _balance = _balance.add(
                    new Decimal(pyPositionsMap[item.id]?.ytBalance || 0).mul(
                        item.ytPrice
                    ) || 0
                )
                ytReward &&
                    (_totalClaim = _totalClaim.add(
                        new Decimal(ytReward?.[item.id] || 0).mul(item.underlyingPrice)
                    ))
            })
            filteredLists.lp.forEach((item) => {
                _balance = _balance.add(
                    new Decimal(lpPositionsMap[item.id]?.lpBalance || 0).mul(
                        item.lpPrice
                    ) || 0
                )
                lpReward &&
                    marketStates[item.marketStateId]?.rewardMetrics.forEach(
                        (rewardMetric) => {
                            _totalClaim = _totalClaim.add(
                                new Decimal(lpReward[item.id + rewardMetric.tokenName]).mul(
                                    rewardMetric.tokenPrice
                                )
                            )
                        }
                    )
