...
export default function Assets({
  ...
  useEffect(() => {
    const flat: RankedPortfolioItem[] = (
      ["pt", "yt", "lp"] as ListOrigin[]
    ).flatMap((key) =>
    ...
    const ranklist = flat.sort((a, b) => {
      ...
      let bValue
      if (a.listType == "lp")
        aValue = new Decimal(lpPositionsMap?.[a.id]?.lpBalance || 0).mul(
          a.lpPrice
        )
      else if (a.listType == "yt")
        aValue = new Decimal(pyPositionsMap?.[a.id]?.ytBalance || 0).mul(
          a.ytPrice
        )
      else
        aValue = new Decimal(pyPositionsMap?.[a.id]?.ptBalance || 0).mul(
          a.ptPrice
        )

      if (b.listType == "lp")
        bValue = new Decimal(lpPositionsMap?.[b.id]?.lpBalance || 0).mul(
          b.lpPrice
        )
      else if (b.listType == "yt")
        bValue = new Decimal(pyPositionsMap?.[b.id]?.ytBalance || 0).mul(
          b.ytPrice
        )
      else
        bValue = new Decimal(pyPositionsMap?.[b.id]?.ptBalance || 0).mul(
          b.ptPrice
        )
  ...
  return (
    <div className="mt-10 mx-7.5 bg-[rgba(252,252,252,0.03)] py-6 px-6 rounded-[24px]">
      {/* Tabs */}
      <div className="flex gap-8 items-center">
        <div className="text-[32px] font-serif font-normal [text-shadow:0_0_32px_rgba(239,244,252,0.56)] text-[#FCFCFC]">
          Assets
        </div>
      ...
      <div>
        <table
          className={`min-w-full border-collapse  border-separate ${
            loading ? "border-spacing-y-4" : "border-spacing-y-0.5"
          }`}
        >
          <thead className="text-white/60">
            <tr>
              <th className="py-2  w-[180px] text-left text-[12px] font-[650]">
                ASSET
              </th>
              <th className="py-2  w-[160px] text-left text-[12px] font-[650]">
                TYPE
              </th>
              <th className="py-2  w-[150px] text-left text-[12px] font-[650]">
                MATURITY
              </th>
              <th className="py-2  w-[100px] text-left text-[12px] font-[650]">
                PRICE
              </th>
              <th className="py-2 text-left text-[12px] font-[650]">AMOUNT</th>
              <th className="py-2 text-left text-[12px] font-[650]">
                INCENTIVE
              </th>
              <th className="py-2 text-left text-[12px] font-[650]">
                ACCRUED YIELD
          ...
          <tbody className="bg-[#0f151c] ">
            {!loading &&
              list.map((item, index) => {
                let totalReward = 0
                if (lpReward)
                  marketStates[item.marketStateId]?.rewardMetrics.forEach(
                    (rewardMetric) => {
                      totalReward =
                        totalReward +
                        Number(lpReward[item.id + rewardMetric.tokenName]) *
                          Number(rewardMetric.tokenPrice)
                    }
                  )
                if (
                  ...
                  return (
                    <tr key={index} className="">
                      ...
                      <td className="py-3">
                        <div
                          className={`text-[12px] font-[650] py-1 px-1.5 rounded-[8px] inline-flex 
                                ${
                                  isExpired(item.maturity)
                      ...
                      <td className="py-3 font-[550] text-[14px] text-[#FCFCFC]">
                        {formatTVL(item.ptPrice)}
                      </td>
                      <td className="py-3 font-[550] text-[14px] text-[#FCFCFC] ">
                        <span>
                          {new Decimal(
                            pyPositionsMap?.[item.id]?.ptBalance || 0
                          ).lt(0.01) && "<"}
                          {formatPortfolioNumber(
                            pyPositionsMap?.[item.id]?.ptBalance || 0
                          )}
                      ...
                      <td className={`py-3 text-[14px] font-[550]`}>
                        <div
                          className={` cursor-pointer px-2.5 py-1.5 items-center rounded-[16px]
                        ...
                        >
                          {isExpired(item.maturity) ? (
                            ...
                            <Link
                              ...
                              href={`/market-detail/${item.id}/${item.coinType}/trade/fixed`}
                            >
                  ...
                  return (
                    <tr key={index} className="">
                      ...
                      <td className="py-3 font-[550] text-[14px] text-[#FCFCFC]">
                        {formatTVL(item.ytPrice)}
                      </td>
                      <td className="py-3 font-[550] text-[14px] text-[#FCFCFC]">
                        <span>
                          {new Decimal(
                            pyPositionsMap?.[item.id]?.ytBalance || 0
                          ).lt(0.01) && "<"}
                          {formatPortfolioNumber(
                            pyPositionsMap?.[item.id]?.ytBalance || 0
                          )}
                      ...
                      <td className="py-3 font-[550] text-[14px] text-[#FCFCFC]">
                        <span>
                          {new Decimal(ytReward?.[item.id] || 0)
                            .mul(item.conversionRate)
                            .lt(0.01) && "<"}
                          {formatPortfolioNumber(
                            new Decimal(ytReward?.[item.id] || 0).mul(
                              item.conversionRate
                            )
                          )}
                        </span>
                        ...
                        >
                          {
                            <Link
                              className="inline-flex gap-1 items-center"
                              href={`/market-detail/${item.id}/${item.coinType}/trade/yield`}
                            >
                              <ArrowUpRight className="w-4 h-4" />
                              Trade
                            </Link>
                  ...
                  return (
                    <tr key={index} className="">
                      ...
                      <td className="py-3 font-[550] text-[14px] text-[#FCFCFC]">
                        <span>
                          {new Decimal(
                            lpPositionsMap?.[item.id]?.lpBalance || 0
                          ).lt(0.01) && "<"}
                      ...
                      <td className="py-3 text-[#FCFCFC] relative group">
                        <div>
                          <div
                            className={`text-[12px] font-[650] py-1 px-1.5 rounded-[8px] cursor-pointer inline-flex 
                                ${
                                  totalReward > 1000
                        ...
                        </div>
                        {
                          ...
                          >
                            ...
                            <div className="flex justify-between text-[14px] font-[550]">
                              <div className="text-[rgba(252,252,252,0.50)]">
                                {"Total"}
                              </div>
            ...
            {loading &&
              [0, 0, 0, 0, 0, 0].map((item, index) => (
                <tr
                  key={index}
                  className="w-full h-[42px] rounded-[15px] bg-gradient-to-r from-[rgba(38,48,66,0.5)] to-[rgba(15,23,33,0.5)] mt-4 overflow-hidden"
                >
                  <td></td>
                  <td></td>
                  <td></td>
