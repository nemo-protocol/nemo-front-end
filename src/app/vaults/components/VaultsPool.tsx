...
// 你的原始代码 —— 省略 import 与 SortIcon 定义
export default function VaultsPool() {
  const [activeTab, setActiveTab]   = useState(categories[0])
  const [search, setSearch]         = useState('')
  const [sortKey, setSortKey]       = useState<'default' | 'apy' | 'tvl'>('default')
  const [sortOrder, setSortOrder]   = useState<'asc' | 'desc'>('desc')

  // 过滤 + 排序得到最终 list。真实场景可以放到 SWR/useQuery 回调里
  const list = useMemo(() => {
    let data = MOCK_DATA
        ...
        <table className="w-max min-w-[calc(100vw-128px)] border-collapse border-spacing-y-0.5">
          ...
          <tbody className="bg-[#0F151C]">
            ...
            {!loading && list.map((item, idx) => (
              <tr key={item.key} className="h-[48px] hover:bg-white/5">
                {/* VAULT 名称 + icon */}
                <td className="py-2">
