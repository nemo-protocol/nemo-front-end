...
export const getPtOutForExactSyIn = <T extends boolean = false>({
  ...
  priceVoucher,
  returnDebugInfo,
}: GetPtOutForExactSyInParams<T>): T extends true ? [TransactionResult, MoveCallInfo] : TransactionResult => {
  ...
  if (returnDebugInfo) {
    debugLog("get_pt_out_for_exact_sy_in_with_price_voucher move call:", moveCallInfo)
  }
...
export const redeemPt = <T extends boolean = false>({
  ...
  pyPosition,
  returnDebugInfo,
}: RedeemPtParams<T>): T extends true ? [TransactionResult, MoveCallInfo] : TransactionResult => {
  const moveCallInfo: MoveCallInfo = {
  ...
  if (returnDebugInfo) {
    debugLog("redeem_pt move call:", moveCallInfo)
  }
...
export const burnPt = <T extends boolean = false>({
  ...
  coinConfig,
  returnDebugInfo,
}: BurnPtParams<T>): T extends true ? [TransactionResult, MoveCallInfo] : TransactionResult => {
  const moveCallInfo: MoveCallInfo = {
  ...
  if (returnDebugInfo) {
    debugLog("burn_pt move call:", moveCallInfo)
  }
